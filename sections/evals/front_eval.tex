...
\label{cha:eval}
{\colorGreen
To gain a thorough understanding of the effectiveness of our preliminary end-to-end agent system, we ran four sets of evaluations as follows.
\begin{itemize}
    \item Performance evaluation of RepoGraph, the frontend context retrieval algorithm, across models of difference sizes and specialties. This is to see consistency, robustness and portability of the Algorithm when under resource constraints and/or the backend model's reasoning capability is limited.
    \item Latency evaluation for CacheBlend, the backend KV cache reuse module, specially under agentic workloads. This differentiates from original work's evaluations with QA and text-based RAG workloads, and is critical for understanding the applicability of the caching scheme.
    \item Performance evaluation of the end-to-end system \frank{this does not exist yet, blending seem to affect output accuracy drastically, working on a fix}
    \item Latency evaluation of the end-to-end system \frank{also DNE yet, given latency improvement issue not resolved}
\end{itemize}}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{graphs/visualized_eval_mini_results_all.png}
\vspace{-0.2in}
